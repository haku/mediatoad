...
public class DbCleanerTest {
	...
	public void itMarksMissingFileAsMissing() throws Exception {
		...
		try (final WritableMediaDb w = this.mediaDb.getWritable()) {
			w.storeFileData(i1.getFile(), new FileData(12, 123456, "myhash", i1.getId(), BigInteger.ZERO, false));
			w.storeFileData(i2.getFile(), new FileData(13, 123457, "myhash2", i2.getId(), BigInteger.ZERO, false));
			w.storeFileData(f3, new FileData(12, 123458, "myhash", i1.getId() + "-dupe", BigInteger.ZERO, false));
		}
