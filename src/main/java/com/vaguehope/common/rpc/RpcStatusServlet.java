...
public class RpcStatusServlet extends HttpServlet {
	...
	protected void doGet(final HttpServletRequest req, final HttpServletResponse resp) throws ServletException, IOException {
		...
		w.println("<h2>requests</h2>");
		final Builder<String, Status.Code, String> tableBuilder = ImmutableTable.builder();
		for (final Entry<String, ClientRecorder> cm : RpcMetrics.clientMetrics()) {
			for (final Entry<String, MethodMetrics> mm : cm.getValue().methodAndMetrics()) {
				...
				for (final Entry<Status.Code, AtomicInteger> sc : mm.getValue().statusAndCount()) {
					tableBuilder.put(rowKey, sc.getKey(), sc.getValue().toString());
				}
			}
		}
		tableToHtml(w, tableBuilder.build());
